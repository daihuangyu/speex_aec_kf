...
class MDF:
    def __init__(self, fs: int, frame_size: int, filter_length: int) -> None:
        ...
        self.P = np.full((self.frame_size + 1, M, 2), 10, dtype=np.float64)
        self.X = np.zeros((self.frame_size + 1, K, M), dtype=np.complex)
        self.Y = np.zeros((self.frame_size + 1, C), dtype=np.complex)
        self.E = np.zeros((self.frame_size + 1, C), dtype=np.complex)
        self.W = np.zeros((self.frame_size + 1, K, M, C), dtype=np.complex)
        self.foreground = np.zeros((self.frame_size + 1, K, M, C), dtype=np.complex)
        ...
        self.nb_adapt = np.zeros(C)
        self.min_count = np.zeros(C)
        self.echo_state = 1# 前处理指针，这里用flag代替
        self.do_dereverb = 0
        self.residual_echo = np.zeros((self.frame_size+1, 1),)
        self.echo_noise = np.zeros((self.frame_size+self.nbands, C), )
        ...
        for i in range(self.frame_size):
            ...
            self.inbuf = np.zeros((N3, C), )
            self.outbuf = np.zeros((N3, C), )
            self.ps = np.zeros((self.frame_size+self.nbands, 1), )
            self.S = np.zeros((self.frame_size, C),)
            self.Smin = np.zeros((self.frame_size, C),)
            self.Stmp = np.zeros((self.frame_size, C), )
            self.update_prob = np.ones((self.frame_size, 1),)
            self.noise = np.ones((self.frame_size+self.nbands, C),)
            self.reverb_estimation = np.zeros((self.frame_size+self.nbands), )
            self.old_ps = np.ones((self.frame_size+self.nbands, C),)
            ...
            self.zeta = np.zeros((self.frame_size+self.nbands, C), )
    ...
    def main_loop(self, u, d):
        ...
        for n in range(0, end_point, self.frame_size):
            ...
            out = self.speex_echo_cancellation_mdf(d_frame[:, ], u_frame[:, None])
            for i in range(self.C):
                out_tmp = self.speex_r2_preprocess_run(np.expand_dims(out[:, i], axis=1), i)
                e[n:n + self.frame_size, i] = out_tmp.squeeze(1)
    ...
    def speex_echo_cancellation_mdf(self, mic, far_end):
        ...
        for speak in range(K):
            self.X[:, speak, 0] = np.fft.rfft(self.x[:, speak]) / N
            self.X[0, speak, 0] = 0
        ...
        for chan in range(C):
            ...
            self.e[:, chan] = np.fft.irfft(self.Y[:, chan]).real * N
        ...
        if self.saturated == 0:
            ...
            for chan in range(C):
                ...
                E = np.fft.rfft(e_fft)/N
                E[0] = 0
                for speak in range(K):
                    X2 = np.expand_dims(np.sum(np.abs(self.X[:, speak, :]) ** 2, axis=1), 1)
                    Pe = 0.5 * self.P[:, :, chan] * X2 + np.expand_dims(np.abs(E) ** 2 / self.M, 1)
                    mu = 0.5 * self.P[:, :, chan] / (Pe + 1e-8)
                    self.P[:, :, chan] = self.A2 * (1 - 0.5 * mu * X2) * self.P[:, :, chan] + (1 - self.A2) * np.abs(self.W[:, speak, :, chan])** 2
            ...
            for speak in range(K):
                for j in range(M):
                    if (self.cancel_count) % M == j:
                        ...
                        self.wtmp = (np.fft.irfft(self.W[:, speak, j, chan]).real)*512
                        self.wtmp[self.frame_size:N] = 0
                        self.W[:, speak, j, chan] = np.fft.rfft(self.wtmp)/512
                        self.W[0, speak, j, chan] = 0
        ...
        for chan in range(C):
            ...
            self.y[:, chan] = np.fft.irfft(self.Y[:, chan]).real * N
        ...
        for chan in range(C):
            ...
            Sdd = Sdd + np.sum(self.input[:, chan] ** 2)

            self.E = np.fft.fft(self.e, axis=0) / N
            self.E[0] = 0
            self.y[:self.frame_size, chan] = 0
            self.Y[:, chan] = np.fft.rfft(self.y[:, chan], axis=0) / N
            self.Y[0] = 0
            self.Rf[:, 0] += np.abs(self.E[:self.frame_size + 1, chan]) ** 2
            self.Yf[:, 0] += np.abs(self.Y[:self.frame_size + 1, chan]) ** 2
        if (not (Syy >= 0 and Sxx >= 0 and See >= 0)) or (not (Sff < N*4.4e12 and Syy < C*N*4.4e12 and Sxx < N*4.4e12)):
            self.screwed_up = self.screwed_up + 50
            out = np.zeros_like(out)
        elif Sff > Sdd + N * 640000:
    ...
    def speex_r2_preprocess_run(self, out, mic_idx):
        N = self.ps_size
        ...
        self.nb_adapt[mic_idx] += 1
        if self.nb_adapt[mic_idx] > 20000:
            self.nb_adapt[mic_idx] = 20000
        self.min_count[mic_idx] += 1

        beta = max(.03, 1/self.nb_adapt[mic_idx])
        beta_1 = 1 - beta
        M = self.nbands
        if self.echo_state:
            self.residual_echo = np.floor(self.r2_echo_get_residual(mic_idx=mic_idx))
            if not (self.residual_echo[0] >= 0 and self.residual_echo[0] < N*1e9):
                self.residual_echo = np.zeros((self.frame_size,),)
            for i in range(N):
                self.echo_noise[i, mic_idx] = max(0.6 * self.echo_noise[i, mic_idx], self.residual_echo[i])
            self.echo_noise[N:, mic_idx] = self.filterbank_r2_compute_bank(self.echo_noise[:, mic_idx]).squeeze(1)
        else:
            self.echo_noise[:, mic_idx] = np.zeros((self.frame_size+self.nbands, ), )
        self.preprocess_analysis(out, mic_idx)
        self.update_noise_prob(mic_idx)
        # Update the noise estimate for the frequencies where it can be
        for i in range(N):
            if not self.update_prob[i] or self.ps[i] < self.noise[i, mic_idx]:
                self.noise[i, mic_idx] = max(0, beta_1*self.noise[i, mic_idx]) + beta * self.ps[i]

        self.noise[N:, mic_idx] = self.filterbank_r2_compute_bank(self.noise[:, mic_idx]).squeeze(1)
        # Special case for first frame
        if self.nb_adapt[mic_idx] == 1:
            for i in range(N+M):
                self.old_ps[i, mic_idx] = self.ps[i] # ps这里可能需要改成局部变量，如果改成多通道
        if self.do_dereverb:
            # Todo:完成去混响
            return 0
        # Compute a posteriori SNR
        for i in range(N+M):
            tot_noise = 1+self.noise[i, mic_idx] + self.echo_noise[i, mic_idx] + self.reverb_estimation[i]
            ...
            gamma = .1 + .89*(self.old_ps[i, mic_idx]/(self.old_ps[i, mic_idx]+tot_noise))**2
            # A priori SNR update = gamma*max(0,post) + (1-gamma)*old/noise
            self.prior[i] = gamma*max(0, self.post[i]) + (1-gamma)*self.old_ps[i, mic_idx]/tot_noise
        ...
        self.zeta[0, mic_idx] = .7 * self.zeta[0, mic_idx] + .3 * self.prior[0]
        for i in range(1, N-1, 1):
            self.zeta[i, mic_idx] = .7 * self.zeta[i, mic_idx] + .15 * self.prior[i] + .075 * self.prior[i-1] + .075 * self.prior[i+1]

        for i in range(N-1, N+M, 1):
            self.zeta[i, mic_idx] = .7 * self.zeta[i, mic_idx] + .3 * self.prior[i]

        # Speech probability of presence for the entire frame is based on the average filterbank a priori SNR
        Zframe = 0
        for i in range(N, N+M, 1):
            Zframe += self.zeta[i, mic_idx]
        ...
        self.gain_floor[N:] = self.comput_gain_floor(-5, effective_echo_suppress, self.noise[N:, mic_idx], self.echo_noise[N:, mic_idx], M)
        ...
        for i in range(N, N+M, 1):
            ...
            self.old_ps[i, mic_idx] = .2*self.old_ps[i, mic_idx] + .8*self.gain[i]**2 * self.ps[i]
            P1 = .199 + .8 * 1/(1+.15/(self.zeta[i, mic_idx]))
            q = 1 - Pframe * P1
        ...
        # Synthesis window (for WOLA)
        for i in range(N3):
            out[i] = self.outbuf[i, mic_idx] + self.frame[i]

        out[N3:N3+N4] = self.frame[N3:N3+N4]
        self.outbuf[:N3, mic_idx] = self.frame[self.frame_size:self.frame_size+N3, 0]

        return out

    def preprocess_analysis(self, x, mic_idx):
        N = self.ps_size
        N3 = 2*N - self.frame_size
        N4 = self.frame_size - N3
        for i in range(N3):
            self.frame[i] = self.inbuf[i, mic_idx]
        for i in range(self.frame_size):
            self.frame[N3+i] = x[i]
        for i in range(N3):
            self.inbuf[i, mic_idx] = x[N4+i]
        for i in range(2*N):
            self.frame[i] = self.frame[i] * self.window2[i]
    ...
    def update_noise_prob(self, mic_idx):
        N = self.ps_size
        for i in range(1, N-1, 1):
            self.S[i, mic_idx] = .8 * self.S[i, mic_idx] + .05 * self.ps[i-1] + .1 * self.ps[i] + .05 * self.ps[i+1]
        self.S[0, mic_idx] = .8 * self.S[0, mic_idx] + .2 * self.ps[0]
        self.S[N-1, mic_idx] = .8 * self.S[N-1, mic_idx] + .2 * self.ps[N-1]

        if self.nb_adapt[mic_idx] == 1:
            self.Smin[:, mic_idx] = np.zeros((N,), )
            self.Stmp[:, mic_idx] = np.zeros((N,), )

        if self.nb_adapt[mic_idx] < 100:
            min_range = 15
        elif self.nb_adapt[mic_idx] < 1000:
            min_range = 30
        elif self.nb_adapt[mic_idx] < 10000:
            min_range = 30
        else:
            min_range = 30
        if self.min_count[mic_idx] > min_range:
            self.min_count[mic_idx] = 0
            for i in range(N):
                self.Smin[i, mic_idx] = min(self.Stmp[i, mic_idx], self.S[i, mic_idx])
                self.Stmp[i, mic_idx] = self.S[i, mic_idx]
        else:
            for i in range(N):
                self.Smin[i, mic_idx] = min(self.Smin[i, mic_idx], self.S[i, mic_idx])
                self.Stmp[i, mic_idx] = min(self.Stmp[i, mic_idx], self.S[i, mic_idx])

        for i in range(N):
            if .4*self.S[i, mic_idx] > self.Smin[i, mic_idx]:
    ...
    def r2_echo_get_residual(self, mic_idx=0):
        N = self.window_size
        for i in range(N):
            self.y_1[i] = self.window[i] * self.last_y[i, mic_idx]
        self.Y_1 = np.fft.rfft(self.y_1, axis=0)/N
        residual_echo = np.zeros((self.frame_size + 1,), )
        residual_echo[1:] = np.abs(self.Y_1[1:])**2
...
if __name__ == "__main__":
    ...
    index = "nhtm_1"
    # _, mic1 = wavfile.read(f"./pcm/{index}_output_mic1.wav")
    # _, mic2 = wavfile.read(f"./pcm/{index}_output_mic2.wav")
    # _, ref = wavfile.read(f"./pcm/{index}_output_ref.wav")
    mic1, sr = sf.read(f"./pcm/{index}_output_mic1.wav")
    mic2, sr = sf.read(f"./pcm/{index}_output_mic2.wav")
    ref, sr = sf.read(f"./pcm/{index}_output_ref.wav")
    min_len = min(len(mic1), len(ref))
    mic1 = mic1[:16000*30]
    mic2 = mic2[:16000 * 30]
    ref = ref[:16000*30]
    mic = np.concatenate((np.expand_dims(mic1, 1), np.expand_dims(mic2, 1)), axis=1)
    # 64 2048 for 8kHz.
    processor = MDF(16000, 256, 3072)
    e = processor.main_loop(ref, mic)

    sf.write(f'./output_1/{index}_traditional3_py.wav', e[:, 0], sr)
    # sf.write('y.wav', y, sr)
