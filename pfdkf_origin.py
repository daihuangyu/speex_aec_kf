...
def pfdkf(x, d, N=12, M=256, A=0.999, P_initial=10, keep_m_gate=0.1, res=False):
    ft = PFDKF(N, M, A, P_initial, keep_m_gate, res)
...
if __name__ == "__main__":
    ...
    mic, sr = sf.read("./glass_wav/wav1_mic.wav")
    ref, sr = sf.read("./glass_wav/wav1_ref.wav")
    # for linear senario:
    # error, echo = pfdkf(ref, mic, A=0.999, keep_m_gate=0.2)
    # for non-linear senario:
    error, echo = pfdkf(ref, mic, A=0.999, keep_m_gate=0.1, res=False)
    sf.write("./glass_wav/wav1_kf_2.wav", error, sr)
