...
class MDF:
    def __init__(self, fs: int, frame_size: int, filter_length: int) -> None:
        ...
        self.P = np.full((N, M, 2), 10, dtype=np.float64)
    ...
    def main_loop(self, u, d):
        ...
        for n in range(0, end_point, self.frame_size):
            ...
            out_tmp = self.speex_r2_preprocess_run(np.expand_dims(out[:, 0], axis=1))
            e[n:n + self.frame_size] = out_tmp.squeeze(1)
    ...
    def speex_echo_cancellation_mdf(self, mic, far_end):
        ...
        for chan in range(C):
            ...
            self.Y[:, chan] = np.fft.fft(self.y[:, chan], axis=0) / N
            self.Y[0] = 0
            if chan == 0:
        ...
        if self.adapted:
            self.last_y[self.frame_size:N] = (mic - out)[:, 0]
        return out
...
if __name__ == "__main__":
    ...
    index = "nhtm_1"
    mic1, sr = sf.read(f"../midput/{index}_mic_out1.wav")
    mic2, sr = sf.read(f"../midput/{index}_mic_out2.wav")
    ...
    sf.write(f'./output_1/{index}_traditional3_py.wav', e, sr)
    # sf.write('y.wav', y, sr)
